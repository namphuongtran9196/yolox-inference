...
def main(args):
    ...
    # Preprocess
    img_preproc, ratio = preproc(img, input_size=(args.input_size, args.input_size))
    img_preproc = np.expand_dims(img_preproc, 0).astype(np.float32)

    # Inference
    outputs = model.run(None, {"images": img_preproc})[0]
...
def make_parser():
    ...
    parser.add_argument(
        ...
        "--model_path",
        type=str,
        default="../models/onnx/yolox_s_decode_in_inference.onnx",
    )
    parser.add_argument(
        "-confthre",
        "--conf_threshold",
